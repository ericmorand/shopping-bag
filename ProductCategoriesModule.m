...
@implementation ProductCategoriesModule

#pragma mark -
#pragma mark FKStackView GETTERS

- (NSString *)name {return @"ProductCategories";}
- (NSString *)entityName {return @"ProductCategory";}
...
- (void)addProductCategoryAction:(id)sender
{
	[self addObject];
}
...
{	
	...
	if ( aToolbar == multipleObjectsFilterBar )
	{
		if ( areaIndex == 0 )
		{
			moduleToolbarItemIdentifiers = [NSArray arrayWithObjects:@"AllProductCategories", nil];
		}
	}
	else if ( aToolbar == multipleObjectsToolbar )
	{
		if ( areaIndex == 0 )
		{
			moduleToolbarItemIdentifiers = [NSArray arrayWithObjects:@"AddProductCategory", @"Delete", nil];
		}
	}
...
{
	if ( [anItemIdentifier isEqualToString:@"AllProductCategories"] ) {return @"TRUEPREDICATE";}
